...
export default function RegisterFormContainer({visible, setVisible}: RegisterFormContainerType) {
    useEffect(() => {
        const timeout = setTimeout(() => setVisible([true, false, false]), 100);
    ...
    const onClick = async () => {
        if (password !== confirmPassword) {
            ...
            if (data.code) {
                ...
                const timeout = setTimeout(() => setVisible([false, false, true]), 100);
                return () => clearTimeout(timeout);
            }
        }
    }
        ...
        <Container visible={visible[0]}>
            ...
            <FormWrapper>
                ...
                <InputWrapper>
                    ...
                    <LoginInput placeholder="닉네임을 입력해주세요(10자 이내)" text={nickName} setText={setNickName}
                                inputType="text"/>
                </InputWrapper>
                <InputWrapper>
                    <LoginSubText text="비밀번호를 입력해주세요"/>
                    <LoginInput placeholder="비밀번호를 입력해주세요(8자 이상)" text={password} setText={setPassword}
                                inputType="password"/>
                    <Box/>
                    <LoginInput placeholder="비밀번호 확인" text={confirmPassword} setText={setConfirmPassword}
                                inputType="password"/>
                </InputWrapper>
            </FormWrapper>
